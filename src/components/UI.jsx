...
/* ---- HEADER ---- */
export function TopHeader({ title, subtitle, onLogout }) {
  return (
    <header style={{
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      padding: '18px 28px', background: '#0a0a0a',
      borderBottom: '1px solid #1a1a1a',
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
        <div style={{
          width: 40, height: 40, background: '#111', border: '2px solid #2563EB',
          borderRadius: 10, display: 'flex', alignItems: 'center', justifyContent: 'center',
          fontSize: 20,
        }}>📊</div>
        <div>
          <div style={{ fontWeight: 700, fontSize: 16, fontFamily: 'var(--font-display)' }}>{title}</div>
          <div style={{ fontSize: 12, color: '#666', fontFamily: 'var(--font-mono)' }}>{subtitle}</div>
        </div>
      </div>
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        padding: 30, maxWidth: 600, width: '90%', maxHeight: '90vh', overflowY: 'auto',
      }}>
